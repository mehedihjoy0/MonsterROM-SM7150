...
_DELETE_FROM_WORK_DIR_IF_EXISTS()
{
    ...
    if [ -e "$WORK_DIR/$PARTITION/$FILE" ]; then
...
if [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "qssi" ]]; then
    ...
    IFS=':' read -r -a SOURCE_EXTRA_FIRMWARES_ARRAY <<< "$SOURCE_EXTRA_FIRMWARES"
    DONOR="$(cut -d "/" -f 1,2 -s <<< "${SOURCE_EXTRA_FIRMWARES_ARRAY[0]}")"
    unset SOURCE_EXTRA_FIRMWARES_ARRAY
else
...
_DELETE_FROM_WORK_DIR_IF_EXISTS "system" "system/lib/libepm.so"
...
_DELETE_FROM_WORK_DIR_IF_EXISTS "system" "system/lib/libpersona.so"
...
_DELETE_FROM_WORK_DIR_IF_EXISTS "system" "system/lib64/libpersona.so"
...
if [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "mssi" ]] || [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "qssi" ]]; then
    ...
    if LC_ALL=C git apply --check \
        --directory="$APKTOOL_DIR/system/framework/framework.jar" \
        --unsafe-paths \
        "$MODPATH/vold/framework.jar/0001-Add-token-argument-in-unlockCeStorage.patch" &> /dev/null; then
        APPLY_PATCH "system" "system/framework/framework.jar" \
            "$MODPATH/vold/framework.jar/0001-Add-token-argument-in-unlockCeStorage.patch"
    else
        LOG "- Skipping obsolete vold framework token patch"
    fi
    DECODE_APK "system" "system/framework/services.jar" || return 1
    if LC_ALL=C git apply --check \
        --directory="$APKTOOL_DIR/system/framework/services.jar" \
        --unsafe-paths \
        "$MODPATH/vold/services.jar/0001-Add-token-argument-in-unlockCeStorage.patch" &> /dev/null; then
        APPLY_PATCH "system" "system/framework/services.jar" \
            "$MODPATH/vold/services.jar/0001-Add-token-argument-in-unlockCeStorage.patch"
    else
        LOG "- Skipping obsolete vold services token patch"
    fi
fi
...
# SEC_PRODUCT_FEATURE_KNOX_SUPPORT_SDP (already nuked in some OneUI 8.5 bases)
DECODE_APK "system" "system/framework/framework.jar" || return 1
if LC_ALL=C git apply --check \
    --directory="$APKTOOL_DIR/system/framework/framework.jar" \
    --unsafe-paths \
    "$MODPATH/sdp/framework.jar/0001-Nuke-Knox-SDP.patch" &> /dev/null; then
    APPLY_PATCH "system" "system/framework/framework.jar" \
        "$MODPATH/sdp/framework.jar/0001-Nuke-Knox-SDP.patch"
else
    LOG "- Skipping obsolete Knox SDP framework patch"
fi
DECODE_APK "system" "system/framework/services.jar" || return 1
if LC_ALL=C git apply --check \
    --directory="$APKTOOL_DIR/system/framework/services.jar" \
    --unsafe-paths \
    "$MODPATH/sdp/services.jar/0001-Nuke-Knox-SDP.patch" &> /dev/null; then
    APPLY_PATCH "system" "system/framework/services.jar" \
        "$MODPATH/sdp/services.jar/0001-Nuke-Knox-SDP.patch"
else
    LOG "- Skipping obsolete Knox SDP services patch"
fi
...
if [[ "$SOURCE_PRODUCT_SHIPPING_API_LEVEL" != "$TARGET_PRODUCT_SHIPPING_API_LEVEL" ]]; then
    SMALI_PATCH "system" "system/framework/services.jar" \
        ...
        "$SOURCE_PRODUCT_SHIPPING_API_LEVEL" \
        > /dev/null
    if awk -v API="$TARGET_PRODUCT_SHIPPING_API_LEVEL" '
        /^\.method.*<init>\(\)V/ { inside = 1 }
        inside && index($0, API) { found = 1 }
        inside && /^\.end method/ { inside = 0 }
        END { exit !found }
    ' "$APKTOOL_DIR/system/framework/services.jar/smali/com/android/server/enterprise/hdm/HdmVendorController.smali"; then
        SMALI_PATCH "system" "system/framework/services.jar" \
        ...
        LOG "- Skipping obsolete Knox HDM vendor API patch"
    fi
fi
...
if LC_ALL=C git apply --check \
    --directory="$APKTOOL_DIR/system/framework/services.jar" \
    --unsafe-paths \
    "$MODPATH/hdm/services.jar/0001-Nuke-Knox-HDM.patch" &> /dev/null; then
    APPLY_PATCH "system" "system/framework/services.jar" \
        "$MODPATH/hdm/services.jar/0001-Nuke-Knox-HDM.patch"
else
    LOG "- Skipping obsolete Knox HDM services patch"
    SMALI_PATCH "system" "system/framework/services.jar" \
        "smali/com/android/server/enterprise/hdm/HdmSakManager.smali" "return" \
        "isSupported(Landroid/content/Context;)Z" "false"
    SMALI_PATCH "system" "system/framework/services.jar" \
        "smali/com/android/server/enterprise/hdm/HdmService.smali" "return" \
        "isHdmPropNONE()Z" "true"
fi
APPLY_PATCH "system" "system/priv-app/DeviceDiagnostics/DeviceDiagnostics.apk" \
...
if [[ "$TARGET_SECURITY_CONFIG_ESE_CHIP_VENDOR" == "none" ]] && [[ "$TARGET_SECURITY_CONFIG_ESE_COS_NAME" == "none" ]]; then
    BLOCKCHAIN_PATCH="$MODPATH/ese+blockchain/services.jar/0001-Nuke-BlockchainTZService.patch"
else
    BLOCKCHAIN_PATCH="$MODPATH/blockchain/services.jar/0001-Nuke-BlockchainTZService.patch"
fi
DECODE_APK "system" "system/framework/services.jar" || return 1
if LC_ALL=C git apply --check \
    --directory="$APKTOOL_DIR/system/framework/services.jar" \
    --unsafe-paths \
    "$BLOCKCHAIN_PATCH" &> /dev/null; then
    APPLY_PATCH "system" "system/framework/services.jar" \
        "$BLOCKCHAIN_PATCH"
else
    LOG "- Skipping obsolete BlockchainTZService patch"
fi
unset BLOCKCHAIN_PATCH
