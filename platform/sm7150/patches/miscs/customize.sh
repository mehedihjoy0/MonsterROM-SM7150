...
{
    local INIT_RC="$WORK_DIR/vendor/etc/init/hw/init.target.rc"
...
_DROP_INCOMPATIBLE_CIDMANAGER()
{
    ...
    LOG "- Removing S26 CIDManager carrier activation incompatible with sm7150"
...
{
    LOG "- Disabling UFFD GC, virtual Perfetto relay, and RKP paths unsupported by sm7150 vendor"
...
{
    LOG "- Disabling Bluetooth audio offload paths unsupported by sm7150 vendor"
...
{
    LOG "- Removing One UI 9 init writes rejected by the sm7150 kernel"
    ...
    _SED_DELETE_IF_EXISTS "$WORK_DIR/vendor/etc/init/init.sm7150.rc" \
...
{
    LOG "- Disabling bootchecker rescue-party loop for legacy sm7150 boots"
    DELETE_FROM_WORK_DIR "system" "system/etc/init/bootchecker.rc"
}

_DISABLE_ZYGOTE_NEXT_BOOT()
{
    LOG "- Disabling zygote_next on legacy sm7150 vendor stack"
...
_RELAX_VOLD_REBOOT_ON_FAILURE()
{
    ...
    if grep -q "^[[:space:]]*reboot_on_failure " "$VOLD_RC"; then
        LOG "- Removing vold reboot_on_failure on legacy sm7150 vendor stack"
