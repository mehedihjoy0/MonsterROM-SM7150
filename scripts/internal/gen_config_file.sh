...
IS_MONSTERROM_CERT_AVAILABLE()
{
    ...
    local USES_MONSTERROM_CERT="false"
    if [[ "$(sha1sum "$SRC_DIR/security/monsterrom_platform.pk8" 2> /dev/null | cut -d " " -f 1)" == "$PLATFORM_KEY_SHA1" ]] && \
        ...
        USES_MONSTERROM_CERT="true"
    fi

    echo "$USES_MONSTERROM_CERT"
}
# ]
...
#     Boolean which describes whether the device supports Wi-Fi to Cellular.
{
    ...
    echo "ROM_IS_OFFICIAL=\"$(IS_MONSTERROM_CERT_AVAILABLE)\""
